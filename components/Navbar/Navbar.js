...
import { nav } from '../../dictionary/dictionary'

export default () => {

    const { locale, path } = useMenuContext()
    const navLang = nav[locale]
          ...
          <div className="items-center px-4 max-w-screen-lg mx-auto lg:flex">
              <div className="font-bold text-lg flex justify-between items-center py-3.5 lg:block">
                    ...
                    <div className="flex items-center">
                        <li className="block text-base font-normal mx-2 lg:hidden">
                            <LangsDropdown 
                                locale={locale}
                                path={path}
                            />
              ...
              <div className={`flex-1 justify-center items-center pb-3 mt-8 lg:flex lg:pb-0 lg:mt-0 ${ state ? 'block' : 'hidden'}`}>
                    <ul className="order-2 flex flex-col-reverse space-x-0 mb-6 lg:space-x-6 lg:flex-row lg:mb-0">
                        ...
                        <AuthLink isLoggedIn={user?.loggedIn}>
                            <li className="nav-item mt-8 lg:mt-0">
                                <Link href="/signup">
                                    <a className={`${locale == 'ar' ? 'py-3 lg:py-1' : 'py-3'} px-4 text-center text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow block lg:inline`}>
                                        { navLang.signup }
                                    </a>
                                </Link>
                            </li>
                        ...
                        <ProtectedLink isLoggedIn={user?.loggedIn}>
                            <li className="nav-item mt-8 lg:mt-0">
                                <button className="flex-row-reverse w-full text-gray-700 hover:text-gray-900 hover:bg-gray-50 flex items-center justify-between border rounded-md p-2.5 md:border-none md:p-0 md:hover:bg-transparent"
                    ...
                    <div className="flex-1">
                        <ul className="justify-center items-center space-y-8 lg:flex lg:space-x-6 lg:space-y-0">
                            <li className={`${ locale == 'ar' ? ' pl-6' : '' } nav-item text-gray-600 hover:text-indigo-600`}>
                                <Link href="/">
                                    <a className="block">
                                        { navLang.home }
                                    </a>
