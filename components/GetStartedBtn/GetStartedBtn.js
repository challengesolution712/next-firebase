import Link from "next/link";
import { useMenuContext } from '../../context/contextApp'

export default () => {

    const { locale } = useMenuContext()

    return (
        <Link href="/signup">
            <a className="max-w-sm mx-auto mt-8 flex items-center justify-between p-3 rounded-full text-[17px] text-blue-600 font-medium bg-indigo-50 lg:hidden">
                {
                    locale == 'ar' ? 
                    'قم بإنشاء حساب لنشر منشور'
                    :
                    'Sign up to create a post'
                }
                {
                    locale == 'ar' ? (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mx-1" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clipRule="evenodd" />
                        </svg>
                    ) : (
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mx-1" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
                        </svg>
                    )
                }
            </a>
        </Link>
    )
}